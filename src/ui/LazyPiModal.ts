...
import type { AddLocalDirectory, LazyAction, LazyModalResult, LazyPackageSummary, LazyTheme, LoadLazyPackages, RunLazyAction } from "../model/types.js";
...
import { getLazyHelpText } from "./getLazyHelpText.js";
...
type LazyCaptureMode = "search" | "add-local" | null;
...
export class LazyPiModal extends Container implements LazyMode {
	...
	private inputMode: LazyCaptureMode = null;
	private query = "";
	private actionSnapshot: LazyAction = "installed";
	...
	constructor(
		...
		private readonly addLocalDirectory: AddLocalDirectory,
	...
	) { super(); void this.refresh(); }

	/** Handles terminal input for the modal. */
	handleInput(data: string): void {
		if (matchesKey(data, Key.ctrl("c"))) return this.onClose(null);
		if (this.inputMode) return this.handleCaptureInput(data);
		...
		if (data === "5") return this.setAction("local");
		if (matchesKey(data, Key.tab)) return this.shiftAction(1);
		if (matchesKey(data, Key.shift("tab"))) return this.shiftAction(-1);
		if (data === "/") return this.beginCapture("search");
		if (data === "a" && this.action === "local") return this.beginCapture("add-local");
		...
		if (data === "d" && this.action !== "search" && this.action !== "favorites") return void this.runSelectedMutation("delete");
	}
	...
	render(width: number): string[] {
		...
		const helpLines = wrapLazyText(getLazyHelpText(this.action), innerWidth);
		const bodyHeight = Math.max(14, Math.floor((this.tui.terminal.rows ?? 40) * 0.72) - 5 - helpLines.length);
		this.scrollOffset = syncLazyScrollOffset(this.selectedIndex, this.scrollOffset, bodyHeight, this.items.length);
		const listLines = this.items.slice(this.scrollOffset, this.scrollOffset + bodyHeight).map((item, index) => renderLazyRow(this.theme, item, leftWidth, this.scrollOffset + index === this.selectedIndex));
		...
		lines.push(renderLazyFrameLine(this.theme, renderLazyQueryLine(this.theme, this.query, this.inputMode), innerWidth));
		for (const helpLine of helpLines) lines.push(renderLazyFrameLine(this.theme, this.theme.fg("dim", helpLine), innerWidth));
		lines.push(this.theme.fg("borderMuted", `└${"─".repeat(innerWidth)}┘`));
		return lines.map((line) => truncateToWidth(line, dialogWidth, ""));
	}

	/** Moves the list selection by one relative step. */
	private moveSelection(delta: number): void { if (this.items.length === 0) return; this.selectedIndex = clampLazyIndex(this.selectedIndex + delta, this.items.length); this.tui.requestRender(); }
	/** Switches to the next or previous tab. */
	private shiftAction(delta: number): void { const actions = getLazyActions(); const currentIndex = actions.indexOf(this.action); this.setAction(actions[(currentIndex + delta + actions.length) % actions.length]!); }
	/** Switches to one explicit tab. */
	private setAction(action: LazyAction): void { if (this.action === action) return; this.action = action; this.inputMode = null; void this.refresh(); }
	/** Starts one text capture interaction. */
	private beginCapture(mode: Exclude<LazyCaptureMode, null>): void {
		this.inputMode = mode;
		this.actionSnapshot = this.action;
		this.querySnapshot = this.query;
		if (mode === "search") this.action = "search";
		this.query = "";
		this.items = [];
	...
	/** Handles text input while capturing search or local directory text. */
	private handleCaptureInput(data: string): void {
		if (matchesKey(data, Key.escape)) return this.cancelCapture();
		if (matchesKey(data, Key.enter)) return void this.finishCapture();
		...
		const pasted = data.replace(/\x1b\[200~/g, "").replace(/\x1b\[201~/g, "");
		const printable = Array.from(pasted).filter((char) => /^[ -~]$/.test(char)).join("");
		if (printable) return this.updateQuery(this.query + printable);
	}
	/** Cancels the current capture flow. */
	private cancelCapture(): void { const mode = this.inputMode; this.inputMode = null; this.pendingLabel = null; this.query = this.querySnapshot; if (mode === "search") this.action = this.actionSnapshot; void this.refresh(); }
	/** Finishes the current capture flow. */
	private async finishCapture(): Promise<void> {
		if (this.inputMode === "search") { this.inputMode = null; await this.refresh(); return; }
		const source = this.query.trim();
		if (!source) { this.error = "Enter a local Pi package path first."; this.tui.requestRender(); return; }
		this.inputMode = null;
		this.query = "";
		try { await this.startTask(await this.addLocalDirectory(source)); }
		catch (error) { this.error = error instanceof Error ? error.message : String(error); this.tui.requestRender(); }
	}
	/** Updates the active captured input. */
	private updateQuery(query: string): void { this.query = query; this.tui.requestRender(); }
	/** Reloads rows for the active tab. */
	private async refresh(): Promise<void> {
		...
		} finally { if (requestId === this.requestId) { this.loading = false; this.tui.requestRender(); } }
	}
	/** Toggles the selected package as a favorite and refreshes the current tab. */
	private async toggleSelectedFavorite(): Promise<void> {
	...
	/** Opens the selected package repository in a browser. */
	private async openSelectedRepository(): Promise<void> { const item = this.items[this.selectedIndex]; if (item?.repository) await this.onOpenRepository(item); }
	/** Runs one install, update, or delete mutation on the selected row. */
	private async runSelectedMutation(action: "install" | "update" | "delete", scope?: "global" | "local"): Promise<void> {
		const item = this.items[this.selectedIndex];
		if (!item || item.kind === "group" || this.running || this.loading || this.pendingLabel) return;
		try { await this.startTask(await this.runAction(action, item, scope ?? item.installScope ?? "global")); }
		catch (error) { this.error = error instanceof Error ? error.message : String(error); this.tui.requestRender(); }
	}
	/** Starts one background task and handles its lifecycle. */
	private async startTask(task: { label: string; completion: Promise<void> }): Promise<void> {
		this.running = true;
		try { this.pendingLabel = task.label; this.tui.requestRender(); await task.completion; this.onClose("reload"); }
		catch (error) { this.error = error instanceof Error ? error.message : String(error); this.pendingLabel = null; this.tui.requestRender(); }
		finally { this.running = false; }
	}
}
